...
const dbUpsert = async (table, rows) => { try { const data=Array.isArray(rows)?rows:[rows]; const mapped=table==="orders"?data.map(r=>({id:r.id,item:r.item,qty:r.qty,unit:r.unit,note:r.note||"",sid:r.sid||null,done:r.done||false,received:r.received||false,auto_added:r.autoAdded||false,confirmed_at:r.confirmedAt||null,comment:r.comment||null})):data; await supa.from(table).upsert(mapped, {onConflict:"id"}); } catch(e) {console.log("upsert err",table,e);} };
...
export default function App() {
  ...
  const [products,setProducts]=useState(PROD_DEFAULT);
  const [suppliers,setSuppliers]=useState(SUP_DEFAULT);
  const [staffList,setStaffList]=useState(STAFF_DEFAULT);
  const [fixedCosts,setFixedCosts]=useState(FC_DEFAULT);
...
<div style={{gridColumn:"1/-1"}}><div style={{fontSize:9,color:"#6b5a3e",marginBottom:2}}>სახელი</div><input defaultValue={p.name} onBlur={e=>updateProd(p.id,"name",e.target.value)} style={{...inp,width:"100%",boxSizing:"border-box"}}/></div>
...
<div><div style={{fontSize:9,color:"#6b5a3e",marginBottom:2}}>ერთეული</div><select defaultValue={p.unit||"kg"} onChange={e=>updateProd(p.id,"unit",e.target.value)} style={{...inp,width:"100%",boxSizing:"border-box"}}>{["kg","l","pcs","g","ml"].map(u=><option key={u} value={u}>{u}</option>)}</select></div>
<div><div style={{fontSize:9,color:"#6b5a3e",marginBottom:2}}>კატეგ.</div><select defaultValue={p.cat||"ბაკალეა"} onChange={e=>updateProd(p.id,"cat",e.target.value)} style={{...inp,width:"100%",boxSizing:"border-box"}}>{["ბაკალეა","ბოსტნეული","რძის პროდ.","ხორცი","ხილი","პური","სასმელები","ყავა","საყოფაცხოვრებო"].map(c=><option key={c} value={c}>{c}</option>)}</select></div>
</div>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
<span style={{fontSize:10,color:"#8a7355"}}>{altSup?"ალტ: "+altSup.name:""}</span>
<button onClick={()=>setProducts(p2=>p2.filter(x=>x.id!==p.id))} style={{padding:"3px 9px",background:"#ef444420",border:"1px solid #ef444450",borderRadius:5,color:"#ef4444",cursor:"pointer",fontSize:10,fontFamily:"inherit"}}>🗑 წაშლა</button>
</div>
</div>
);
})}
